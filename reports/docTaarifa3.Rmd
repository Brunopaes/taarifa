...
Os dados foram adquiridos pelo site: "Driven Data dentro da competição Pump it Up: Data Mining the Water Table" eles estavam distribuidos em 2 tabelas, o modelo de teste com os dados coletados pelo ministerio de agua da Tanzania e a segunda tabela com o modelo de treinamento para as bombas de agua.E por fim, uma tabela com o modelo de submissão dos resultados obtidos pelo processamento. Atribuimos a tabela de treinamento como variavel "Y"" e a tabela de teste nós dividimos em 2 modelos de variavel "X", um para teste e outro para treino.  
...
```
Estes atributos, aparentemente, apresentam a mesma informacao, não fazendo muito sentido então utilizar os 2 atributos para fazer a analise.
...
```
Dessa forma o atributo __quantity_group__ foi removido devido a sua forma redundancia e maior extensão de nome.
...
```
Embora o significado de todos os sete atributos sejam os mesmos, o atributo __basin__ , quando comparado com os outros seis atributos, é menos complexo (9 valores Unicos). Portanto, o atributo __basin__ consegue melhor generalizar o _dataset_. 
...
```
Este atributo, por ter um intervalo de valores muito alto, não generaliza o conjunto de dados.
...
```
Este atributo, é bastante importante. No entanto, este atributo é um pouco inconsistente, uma vez que, em 21381 registros, a população circundante é 0.
...
```
Este atributo, é um dos mais importantes. Por isso, não deve ser removido. No entanto, existem valores inconsistentes (20709 registros iguais a 0).
Estes registros, ou devem ser tratados ou removidos do conjunto de dados.

******

#### Conclusão Pré-Processamento
Random Forests não podem lidar com mais de 53 classes categóricas!

1a Approach: remova o instalador e o construction_year.

2a Approach: Se esta decisão impactar diretamente na precisão do modelo, variáveis dummy serão construídas.
...
Utilizando o modelo Random Forest em grid search e hiperparametrização, obtivemos uma acuracia máxima de 86% no dataset de treinamento. O modelo, no dataset de testes, obteve uma acurácia máxima de 78%. Embora a acurácia, quando comparada com as predições feitas no dataset de testes, tenha sido inferior (caracterizando um overfitting do modelo), o modelo obteve um bom desempenho. Uma das formas de se constatar sua eficiência é comparar o seu desempenho com os outros participantes da competição (A acurácia máxima no dataset de validação foi de 82,85%). Pode-se notar que a diferença entre o primeiro lugar da competição (82,85%) e o modelo do grupo (78,63%) é menor que 5%.
